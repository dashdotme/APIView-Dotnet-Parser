...
namespace APIView.DotnetParser
{
    public class CLanguageService : LanguageService
    {
        ...
        public override async Task<CodeFile> GetCodeFileAsync(string originalName, Stream stream, bool runAnalysis)
        {
            ...
            var archive = new ZipArchive(astStream, ZipArchiveMode.Read, false, Encoding.UTF8);
            ...
            foreach (var entry in archive.Entries)
            {
                ...
                entryStream = ToUTF8NoBom(entryStream);
        ...
        private static MemoryStream ToUTF8NoBom(MemoryStream stream)
        {
            ReadOnlySpan<byte> Utf8Bom = new byte[] { 0xEF, 0xBB, 0xBF };
            ReadOnlySpan<byte> Utf16Bom = new byte[] { 0xFF, 0xFE };


            MemoryStream MemStream;

            if (stream.GetBuffer().AsSpan().StartsWith(Utf16Bom))
                MemStream = ParseUTF16toUTF8(stream);
            else
            {
            ...
            if (MemStream.GetBuffer().AsSpan().StartsWith(Utf8Bom))
            {
                byte[] precut = MemStream.GetBuffer();
        ...
        private static MemoryStream ParseUTF16toUTF8(MemoryStream stream)
        {
            ...
            byte[] Utf8BytesNoNull = new byte[eof + 1];
            Array.Copy(Utf8Bytes, Utf8BytesNoNull, eof + 1);

            return new MemoryStream(Utf8BytesNoNull, 0, Utf8BytesNoNull.Length, true, true);
        }
